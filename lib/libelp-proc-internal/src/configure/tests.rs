...
// Utility: Build DeriveInput with fields
fn make_struct(struct_name: &str, fields: Vec<(&str, Type, Option<Expr>, Option<&str>)>) -> DeriveInput {
    // Use syn to construct a struct token and then parse as DeriveInput for better reliability
    ...
    for (name, ty, default_expr, note) in fields {
        ...
        if let Some(expr) = default_expr {
            let _ts = quote! { #expr }.to_string();
            // Direct text insertion would include quotes/suffixes, use parse_quote! to simplify
...
fn test_handler_non_struct_error() {
    // enum not supported, should return compile error token
...
fn test_handler_generates_impls_for_primitives_and_nested() {
    // Top level contains basic types and a child struct, verify __ELP_CHILD_DEPTH, new, to_toml/from_toml generation
    ...
    let _ = handler(child); // Just generate, don't assert text
...
fn test_handler_depth_assert_over_two_levels_panics() {
    // Three-level nesting: Top -> Mid -> Leaf, should include panic text in constant calculation
    ...
    let ts = handler(top);
    let s = ts.to_string();
    assert!(s.contains("nesting level exceeds allowed two levels") || s.contains("__ELP_ASSERT_MSG"));
}

#[test]
fn test_toml_utils_field_value_and_generate_impls() {
    // Verify field_value_to_toml_string branches for default values and types; indirectly cover generate_to_toml_impl
    ...
    // Generate to_toml / from_toml code
...
fn test_toml_utils_field_value_default_none_by_type_and_non_literal_expr() {
    ...
    // Non-literal expression -> default_value
...
fn test_generate_to_toml_impl_error_on_unnamed_field() {
    // Construct field without ident (tuple field)
    ...
    assert!(msg.contains("Field must have a name"));
}

#[test]
fn test_handler_error_from_invalid_default_literal() {
    // Use character literal for String, trigger process_default_value error and bubble up as compile error token in handler
    let di: DeriveInput = make_struct(
        "Bad",
        vec![
    ...
    let ts = handler(di);
    let s = ts.to_string();
    assert!(s.contains("Unsupported literal type") || s.contains("compile_error"));
}
